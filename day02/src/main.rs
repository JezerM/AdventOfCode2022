...
fn calculate_score(my_choice: &Choice, opp_choice: &Choice) -> u32 {
    ...
    score += match game_state {
        GameState::Draw => {
            println!("Draw - {:?} - {:?}", my_choice, opp_choice);
            3
        }
        GameState::Win => {
            println!("Win - {:?} - {:?}", my_choice, opp_choice);
            6
        }
        GameState::Lose => {
            println!("Lose - {:?} - {:?}", my_choice, opp_choice);
            0
        }
    };

    println!("\tSum score: {score}");
...
fn calculate_choice_and_score(how_to_end: &EndChoice, opp_choice: &Choice) -> u32 {
    ...
    println!("Choice: {:?} - {:?}", how_to_end, my_choice);
